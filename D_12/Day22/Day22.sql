...
group by MovieID having count(PersonID) >= 2 order by count(*) desc;
...
-- 부학장님과 함께
select movie.KoreanTitle
from person join appear on person.PersonID = appear.PersonID
join movie on appear.MovieID = movie.MovieID
join role on appear.RoleID = role.RoleID
where RoleKorName = '감독'
and KoreanName = '제임스 카메론'
intersect
select movie.KoreanTitle
from person join appear on person.PersonID = appear.PersonID
join movie on appear.MovieID = movie.MovieID
...
inner join role ON role.RoleID = appear.RoleID
inner join person ON person.PersonID = appear.PersonID
where (GenreName == 'Horror' and RoleName = 'Actor');
...
-- , GenreName 
from movie
natural join moviegenre natural join genre natural join appear 
natural join role natural join person
where (GenreName = 'Drama' and RoleName = 'Actor')
or (GenreName != 'Horror' and RoleName = 'Actor')
group by Name
-- order by Name
) as a
;
